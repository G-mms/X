...
apt-get install apache2 ufw -y > /dev/null 2>&1
printf "DONE!\n"
...
tar cf - ./ChainDB -P | pv -s $(du -sb ./ChainDB | awk '{print $1}') | gzip > ChainDB.tar.gz
...
# symlink file to web host dir
sudo ln -sr ChainDB.tar.gz /var/www/html/ChainDB.tar.gz > /dev/null 2>&1
sudo rm -f /var/www/html/index.html > /dev/null 2>&1
...
sudo tar zcf - ./ChainDB/ -P | pv -s $(du -sb ./ChainDB | awk '{print $1}') | ssh "$sshusername"@"$sship" "cat > /var/www/html/ChainDB.tar.gz"
...
printf "Next time you install a node, it will use a newer database.\n\n"
...
# Install NKN node miner software and download the ChainDB
...
if [ $database == "no" ]; then
	# script skips DB download and continues
        printf "Waiting for NKN software to start....................................... DONE!\n"
        install3
else
	printf "Waiting for NKN node software to start.................................. "
	timestart=$(date +%s)
	while [[ $(($(date +%s) - $timestart)) -lt 300 ]]; do # 300sec 5 min
	        if [ ! -f /home/$username/nkn-commercial/services/nkn-node/wallet.json ]; then
...
read -t 10 > /dev/null 2>&1
cd /home/$username/nkn-commercial/services/nkn-node/ > /dev/null 2>&1
rm -rf ChainDB/ > /dev/null 2>&1

# if from beginner menu, extract locally, if not download from websource
if [ $mode == "beginner" ]; then
	cd /var/www/html/
	pv ChainDB.tar.gz | tar xzf - -C /home/$username/nkn-commercial/services/nkn-node/ ###############
else
